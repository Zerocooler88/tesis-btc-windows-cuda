...
import argparse  # Procesa argumentos de consola para elegir escenarios y parámetros del baseline.
import importlib.util  # Carga dinámicamente el archivo 10_common_training_pipeline.py.
import json  # Exporta el resumen consolidado del baseline en formato JSON.
...
COMMON_MODULE_NAME = "common_training_pipeline_v10"  # Define un nombre interno estable para registrar el módulo cargado.


def load_common_pipeline_module():
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Ejecuta el baseline de persistencia reutilizando 10_common_training_pipeline.py "
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="Tamaño de lote. Se conserva por compatibilidad con la interfaz común.",
    )  # Mantiene compatibilidad aunque el baseline no entrene red neuronal.
    parser.add_argument(
        "--epochs",
        type=int,
        default=50,
        help="Número máximo de épocas. Se conserva por compatibilidad con la interfaz común.",
    )  # Mantiene compatibilidad total con el pipeline común.
    parser.add_argument(
        "--patience",
        type=int,
        default=5,
        help="Paciencia de early stopping. Se conserva por compatibilidad con la interfaz común.",
    )  # Mantiene compatibilidad total con el pipeline común.
    parser.add_argument(
        "--learning-rate",
        dest="learning_rate",
        type=float,
        default=0.001,
        help="Tasa de aprendizaje. Se conserva por compatibilidad con la interfaz común.",
    )  # Mantiene compatibilidad total con el pipeline común.
    parser.add_argument(
        "--target-mode",
        dest="target_mode",
        type=str,
        default="delta",
        help="Modo de target solicitado por interfaz: 'price' o 'delta'.",
    )  # Conserva la interfaz del proyecto aun cuando el baseline operativo use precio directo.
    parser.add_argument(
        "--num-workers",
        dest="num_workers",
        type=int,
    ...
    )  # Mantiene la reproducibilidad de la preparación del escenario.
...
def build_experiment_config(common: Any, args: argparse.Namespace) -> Any:
    ...
    )  # Valida batch_size por coherencia de interfaz.
    validate_positive_integer(
        args.epochs, "epochs"
    )  # Valida epochs por coherencia de interfaz.
    validate_positive_integer(
        args.patience, "patience"
    )  # Valida patience por coherencia de interfaz.
    validate_positive_float(
        args.learning_rate, "learning_rate"
    )  # Valida learning_rate por coherencia de interfaz.
    validate_non_negative_integer(
        args.num_workers, "num_workers"
    )  # Valida num_workers para Windows.
    common.validate_target_mode(
        args.target_mode
    )  # Valida el target_mode solicitado aunque el baseline operativo use precio directo.
...
def print_header(common: Any, args: argparse.Namespace, scenarios: List[Any]) -> None:
    ...
    print("BASELINE PERSISTENCIA")  # Imprime el encabezado principal del experimento.
    print(
        "experimento        = baseline_persistencia"
    )  # Imprime el nombre estándar del modelo base.
    print(
    ...
    )  # Imprime batch_size por compatibilidad de interfaz.
    print(
        f"epochs             = {args.epochs}"
    )  # Imprime epochs por compatibilidad de interfaz.
    print(
        f"patience           = {args.patience}"
    )  # Imprime patience por compatibilidad de interfaz.
    print(
    ...
    )  # Imprime learning_rate por compatibilidad de interfaz.
    print(
        f"target_mode        = {common.validate_target_mode(args.target_mode)}"
    )  # Imprime el modo pedido por interfaz.
    print(
...
def run_baseline_for_scenario(
    common: Any,
    scenario: Any,
    config: Any,
) -> Dict[str, Any]:
    print(
        f"Ejecutando baseline en escenario: {scenario.display_name}"
    )  # Informa claramente qué escenario está corriendo en este momento.

    prepared = common.prepare_baseline_scenario(
        scenario=scenario,
        config=config,
    )  # Reutiliza la preparación oficial del pipeline común en modo baseline.

    summary = common.export_baseline_predictions(
        prepared=prepared,
        model_name="baseline_persistencia",
        config=config,
    )  # Reutiliza la exportación oficial de predicciones y métricas del pipeline común.
...
def build_consolidated_record(
    ...
    return {
        "modelo": scenario_summary.get("model_name", "baseline_persistencia"),
...
def build_consolidated_payload(
    ...
    return {
        ...
        "config": {
            **config_dict,
            "target_mode_argumento": args.target_mode,
            "target_mode_baseline_operativo": "price",
        },
        "resultados": records,
    }  # Construye el JSON consolidado final con configuración, escenarios y resultados.
...
def build_consolidated_csv_rows(
    ...
    for record in records:
        ...
        flat_rows.append(
            {
                "modelo": record.get("modelo", "baseline_persistencia"),
...
def export_consolidated_summary(
    ...
    json_path = (
        metrics_dir / "baseline_persistencia__resumen_consolidado.json"
    )  # Define la ruta del JSON consolidado final.
    csv_path = (
        metrics_dir / "baseline_persistencia__resumen_consolidado.csv"
...
def main() -> None:
    ...
    config = build_experiment_config(
        common, args
    )  # Construye la configuración del baseline reutilizando la dataclass oficial.
    ...
    for scenario in scenarios:
        try:
            scenario_summary = run_baseline_for_scenario(
                common=common,
                scenario=scenario,
                config=config,
            )  # Ejecuta el baseline del escenario actual usando exclusivamente lógica común.
...
if __name__ == "__main__":
    try:
        main()  # Lanza la ejecución completa del baseline de persistencia.
    except Exception as exc:
        print(
            "ERROR EN 11_baseline_persistencia.py"
