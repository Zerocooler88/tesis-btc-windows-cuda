...
import torch.nn as nn  # Define capas y módulos de la arquitectura CNN1D.
...
COMMON_MODULE_NAME = "common_training_pipeline_v10_for_cnn1d"  # Define un nombre interno estable para registrar el módulo cargado.
MODEL_NAME = (
    "cnn1d"  # Define el nombre oficial del modelo para rutas, métricas y consola.
)
...
DEFAULT_KERNEL_SIZE = (
    3  # Define un kernel pequeño y estable para patrones locales temporales.
)
DEFAULT_DROPOUT = 0.20  # Define un dropout moderado para estabilizar el entrenamiento.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Entrena el modelo CNN1D reutilizando 10_common_training_pipeline.py "
...
def print_header(common: Any, args: argparse.Namespace, scenarios: List[Any]) -> None:
    ...
    print("CNN1D")  # Imprime el encabezado principal del experimento.
    print(
...
class CNN1DRegressor(nn.Module):
    def __init__(
        ...
        )  # Aprende patrones locales temporales y colapsa el eje temporal a una representación fija.
        ...
        self.regression_head = nn.Sequential(
            nn.Linear(conv_channels_2, mlp_hidden_size),
            nn.ReLU(),
            nn.Dropout(p=dropout),
            nn.Linear(mlp_hidden_size, 1),
        )  # Define una cabeza MLP homogénea con LSTM y GRU para una comparación metodológica justa.

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.ndim != 3:
            raise ValueError(
                f"CNN1DRegressor esperaba un tensor 3D con forma [batch, lookback, features]. "
        ...
        features = features.squeeze(
            -1
        )  # Elimina la longitud temporal residual después del pooling adaptativo.
        features = self.output_dropout(
            features
        )  # Aplica dropout antes de la regresión final.
        prediction = self.regression_head(
            features
        )  # Proyecta la representación final a una salida escalar.
        return (
...
def model_builder(input_size: int, lookback: int) -> nn.Module:
    ...
    )  # Construye una instancia CNN1D consistente con el experimento.


def run_cnn1d_for_scenario(
    common: Any,
    scenario: Any,
    config: Any,
...
def build_consolidated_payload(
    ...
    return {
        ...
        "arquitectura_modelo": {
            ...
            "cabeza_salida": "Linear(conv_channels_2, 32) -> ReLU -> Dropout -> Linear(32, 1)",
        },
        "resultados": records,
...
def main() -> None:
    ...
    for scenario in scenarios:
        try:
            scenario_summary = run_cnn1d_for_scenario(
                common=common,
                scenario=scenario,
                config=config,
                train_target_mode=target_mode,
            )  # Ejecuta el entrenamiento CNN1D del escenario actual usando exclusivamente lógica común.
...
if __name__ == "__main__":
    try:
        main()  # Lanza la ejecución completa del experimento CNN1D.
    except Exception as exc:
        print(
            "ERROR EN 14_entrenar_cnn1d.py"
