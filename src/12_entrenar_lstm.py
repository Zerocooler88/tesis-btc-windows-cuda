...
import torch.nn as nn  # Define capas y módulos de la arquitectura LSTM.
...
COMMON_MODULE_NAME = "common_training_pipeline_v10_for_lstm"  # Define un nombre interno estable para registrar el módulo cargado.
MODEL_NAME = (
    "lstm"  # Define el nombre oficial del modelo para rutas, métricas y consola.
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Entrena el modelo LSTM reutilizando 10_common_training_pipeline.py "
...
def print_header(common: Any, args: argparse.Namespace, scenarios: List[Any]) -> None:
    ...
    print("LSTM")  # Imprime el encabezado principal del experimento.
    print(
...
class LSTMRegressor(nn.Module):
    def __init__(
        ...
        )  # Define la LSTM principal que procesa la secuencia temporal completa.
        ...
        )  # Define una cabeza MLP homogénea para comparar de forma justa con GRU.
...
def model_builder(input_size: int, lookback: int) -> nn.Module:
    ...
    )  # Construye una instancia LSTM consistente con el experimento.


def run_lstm_for_scenario(
    common: Any,
    scenario: Any,
    config: Any,
...
def main() -> None:
    ...
    for scenario in scenarios:
        try:
            scenario_summary = run_lstm_for_scenario(
                common=common,
                scenario=scenario,
                config=config,
                train_target_mode=target_mode,
            )  # Ejecuta el entrenamiento LSTM del escenario actual usando exclusivamente lógica común.
...
if __name__ == "__main__":
    try:
        main()  # Lanza la ejecución completa del experimento LSTM.
    except Exception as exc:
        print(
            "ERROR EN 12_entrenar_lstm.py"
